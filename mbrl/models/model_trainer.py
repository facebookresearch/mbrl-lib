...
class DynamicsModelTrainer:
    ...
    def train(
        ...
        Args:
            ...
            bootstrap_permutes (bool): for ensemble models, decides if the bootstrap
                data is generated by permutations (``True``), or by sampling with
                replacement(``False``). Defaults to ``False``.
        ...
        dataset_train, dataset_val = self.replay_buffer.get_iterators(
            batch_size,
            val_ratio,
            train_ensemble=len(self.model) is not None,
            ensemble_size=len(self.model),
            shuffle_each_epoch=True,
            bootstrap_permutes=bootstrap_permutes,
        )
