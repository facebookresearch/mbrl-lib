...
from mbrl.types import TransitionBatch, TrajTransitionBatch
...
class TrajBasedReplayBuffer(ReplayBuffer):
    """A replay buffer with support for storing control parameters and time horizion from initial state. Note that next_obs in this case is a time + horizon, rather than plus one.
    ...
    def __init__(
        ...
        obs_type: Type = np.float32,
        hor_type: Type = np.int_,
        param_type: Type = np.float32,
        reward_type: Type = np.float32,
        rng: Optional[np.random.Generator] = None,
        max_trajectory_length: Optional[int] = None,
    ):
        super().init(
            capacity=capacity,
            obs_shape=obs_shape,
            action_shape=param_shape,
            obs_type=obs_type,
            action_type=param_type,
    ...
    def add(
        ...
        params: np.ndarray,
        next_obs: np.ndarray,
        horizon: Union[np.int_, np.float32],
        reward: float,
        done: bool,
    ):
        """Adds a transition (s, a, s', h, r, done) to the replay buffer.

        Args:
            obs (np.ndarray): the observation at time t.
            params (np.ndarray): the policy parameters at time t.
            next_obs (np.ndarray): the observation at time t + h.
            horizon (int for float): the timestep between current obs and the next obs
            reward (float): the reward at time t.
            done (bool): a boolean indicating whether the episode ended or not.
        """
        self.obs[self.cur_idx] = obs
        self.next_obs[self.cur_idx] = next_obs
        self.action[self.cur_idx] = params
        self.horizon[self.cur_idx] = horizon
        self.reward[self.cur_idx] = reward
        self.done[self.cur_idx] = done
    ...
    def add_batch(
        ...
        Expected shapes are:
            obs --> (batch_size,) + obs_shape
            act --> (batch_size,) + action_shape
            reward/done/horizon --> (batch_size,)
        ...
        def copy_from_to(buffer_start, batch_start, how_many):
            ...
            batch_slice = slice(batch_start, batch_start + how_many)
            np.copyto(self.obs[buffer_slice], obs[batch_slice])
            np.copyto(self.action[buffer_slice], params[batch_slice])
            np.copyto(self.horizon[buffer_slice], horizon[batch_slice])
            np.copyto(self.reward[buffer_slice], reward[batch_slice])
            np.copyto(self.next_obs[buffer_slice], next_obs[batch_slice])
            np.copyto(self.done[buffer_slice], done[batch_slice])
    ...
    def _batch_from_indices(self, indices: Sized) -> TransitionBatch:
        ...
        next_obs = self.next_obs[indices]
        action = self.action[indices]
        horizon = self.horizon[indices]
        reward = self.reward[indices]
        done = self.done[indices]

        return TrajTransitionBatch(obs, action, next_obs, horizon, reward, done)
    ...
    def save(self, save_dir: Union[pathlib.Path, str]):
        ...
        np.savez(
            ...
            obs=self.obs[: self.num_stored],
            next_obs=self.next_obs[: self.num_stored],
            horizon=self.horizon[: self.num_stored],
            action=self.action[: self.num_stored],
            reward=self.reward[: self.num_stored],
            done=self.done[: self.num_stored],
            trajectory_indices=self.trajectory_indices or [],
    ...
    def load(self, load_dir: Union[pathlib.Path, str]):
        ...
        self.obs[:num_stored] = data["obs"]
        self.next_obs[:num_stored] = data["next_obs"]
        self.horizon[:num_stored] = data["horizon"]
        self.action[:num_stored] = data["action"]
        self.reward[:num_stored] = data["reward"]
        self.done[:num_stored] = data["done"]
        self.num_stored = num_stored
